...
class RedditBot:
    ...
    if len(db) == 0:
      with open(filename) as csv_file:
        ...
        for row in csv_reader:
          self.response_list.append({'phrase': clean_string(row[0]), 
                                     'reply': row[1]
          })
  ...
  def cooled_down(self, i):
    ...
    if 'last_posted' not in dictionary.keys():
      #meaning we never posted this comment before
      return True
    else:
      now = datetime.now()
...
# Warning clear all posted times
# Only use it if you want to change phrases replies
# db.clear()
